...
const prisma_1 = require("../src/shared/lib/prisma");
const order_hold_service_1 = require("../src/modules/order/order-hold.service");
const orderHoldService = new order_hold_service_1.OrderHoldService(prisma_1.prisma);
async function main() {
    ...
    const concert = await prisma_1.prisma.concert.create({
        data: {
    ...
    const inventory = await prisma_1.prisma.ticketInventory.create({
        data: {
    ...
    const users = [];
    for (let i = 0; i < 100; i++) {
        users.push(prisma_1.prisma.user.create({
            data: {
    ...
    const finalInventory = await prisma_1.prisma.ticketInventory.findUnique({
    ...
    const orderIds = await prisma_1.prisma.order.findMany({
        where: { concertId: concert.id },
        select: { id: true },
    });
    const ids = orderIds.map((o) => o.id);
    if (ids.length > 0) {
        await prisma_1.prisma.ticket.deleteMany({ where: { orderItemId: { in: (await prisma_1.prisma.orderItem.findMany({ where: { orderId: { in: ids } }, select: { id: true } })).map(oi => oi.id) } } });
        await prisma_1.prisma.orderItem.deleteMany({ where: { orderId: { in: ids } } });
        await prisma_1.prisma.order.deleteMany({ where: { id: { in: ids } } });
    }
    await prisma_1.prisma.ticketInventory.delete({ where: { ticketTypeId: ticketType.id } });
    await prisma_1.prisma.ticketType.delete({ where: { id: ticketType.id } });
    await prisma_1.prisma.concert.delete({ where: { id: concert.id } });
    const userIds = createdUsers.map((u) => u.id);
    await prisma_1.prisma.user.deleteMany({ where: { id: { in: [...userIds, organizer.id] } } });
    await prisma_1.prisma.organization.delete({ where: { id: org.id } });
