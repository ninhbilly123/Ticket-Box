...
- **THEN** UI SHALL chuẩn hóa khoảng trắng đầu cuối và chữ hoa trước khi gửi yêu cầu tạo concert
...
- **AND** UI SHALL hiển thị thông điệp lỗi backend cho ban tổ chức

### Requirement: Điều hướng tích hợp trong dashboard hiện tại
Admin frontend SHALL cung cấp các mục `AI Artist Bio`, `Email nhãn hàng` và `VIP Sync` trong hệ thống tab hiện có mà không làm thay đổi các chức năng Concerts, Ticket Types, Staff, Whitelist và Revenue.
...
- **THEN** UI SHALL tiếp tục hiển thị trạng thái từ chối truy cập hiện có
- **AND** UI SHALL không tải dữ liệu AI Bio, sponsor email hoặc VIP report

### Requirement: Giao diện mới thống nhất với style hiện tại
Các màn hình mới SHALL tái sử dụng ngôn ngữ thiết kế và component hiện có của `admin-frontend`, đồng thời hỗ trợ desktop và mobile mà không chồng lấn nội dung.

#### Scenario: Hiển thị trên desktop
- **WHEN** dashboard hiển thị trên viewport desktop
- **THEN** form, bộ lọc và bảng SHALL dùng panel trắng, viền slate, màu nhấn cyan, icon Lucide và kích thước chữ tương thích các tab hiện tại
...
- **THEN** UI SHALL hiển thị empty state cụ thể cho chức năng đó
...
- **AND** không secret tích hợp nào SHALL xuất hiện trong response model, form hoặc local storage
